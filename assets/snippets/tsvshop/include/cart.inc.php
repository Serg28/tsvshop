...
function session($cache) {
  ...
  if ($cache) {$cache->cache("session", "tsvshop", $session);}
  return $session;
}
}
...
function tsv_add_item($cache, $id, $name, $opt, $icon, $qty = 1, $url, $typeitem) {
  ...
  if (is_numeric($qty) || is_float($qty)) {
	...
	if ($validQty !== false) {
        ...
        if (!$incart) {
	    if(!$icon) {$icon = "assets/snippets/tsvshop/images/noimage.png";}
            $items = array( 'id' => $id,
                            'name' => $name,
                            'articul' => $articul,
        						        'price' => $price,
        ...
        //$evt = $modx->invokeEvent("TSVshopOnAddItem",array("item" => $_SESSION[$session]['orders'][$id]));
...
function tsv_delete_item($num) {
  ...
  if (is_array($_SESSION[$session]['orders'])) {
    ...
    $modx->invokeEvent("TSVshopOnDeleteItem",array("item" => $num));
  ...
  $count = sizeof($_SESSION[$session]['orders']);  
  //if (empty($count)) { 
  //  $goto=$tsvshop['basketurl']; 
  //} else {
  //  $goto=$tsvshop['selfurl'];   
  //}
  //$modx->sendRedirect($goto,0,'REDIRECT_HEADER');
...
function tsv_modify_quantity($num, $quantity) {
  ...
  $modx->invokeEvent("TSVshopOnChangeItemQty",array("item" => $num, "qty"=>$quantity));
...
function tsv_display_infoblock($cache) {
    ...
    if (is_array($tsvshop['hideon'])) {
      if (in_array($modx->documentIdentifier, $tsvshop['hideon'])) return;
    }
    ...
    for ($i = ($count-1); $i >=0; $i--) {
      ...
      if (!$items) $output .= str_replace("[+shop.info.sempty+]", $shop_lang['strEmptyInfo'], $empty);
      if ($items == 1) $output .= str_replace("[+shop.info.stovar+]", $shop_lang['strTovar1'], $full);
      if ($items >= 2 && $items <= 4) $output .= str_replace("[+shop.info.stovar+]", $shop_lang['strTovar2'], $full);
      if ($items > 4) $output .= str_replace("[+shop.info.stovar+]", $shop_lang['strTovar5'], $full);
...
function tsv_display_cart($cache, $act="basket") {
  ...
  //$session = session($cache);
    ...
    $count = sizeof($_SESSION[$session]['orders']);
    if ($act=="basket") {$tvar="ct"; $chunk = $tsvshop['tplcart']; } else {$tvar="ch"; $chunk = $tsvshop['tplcheckout']; $divajax='<div id="checkout_table">';} 
    if (empty($count)) { $tvar="cte"; $chunk = $tsvshop['tplcartempty']; }
 ...
 // создаем метки для корзины
    ...
    foreach ($folders as $folder) {
      if ($folder != "."  && $folder != ".." ) {
        if (isset($tsvshop['addons_'.$folder.'_active']) && $tsvshop['addons_'.$folder.'_active']=="yes") {
          ...
          // удаляем из чанка вообще эту строку для аддона. Она не нужна.
          $piece[$folder] = getStr($tpl, '<!--'.$folder.'-->', '<!--/'.$folder.'-->');
    ...
    */ 
    ...
    if (empty($count)) {      
      ...
      //$tpl = str_replace($noempty, $table, $tpl); // было
      //$tpl = str_replace($empty, "", $tpl);       // было
      ...
      if(is_array($evt) && !empty($evt[0])) $sub = $evt[0];
...
function tsv_Finish(&$fields) {
  ...
  if (!empty($count)) {
     for ($i = ($count-1); $i >=0; $i--) {
        ...
        $price = tsv_CalcPrice($_SESSION[$session]['orders'][$i]['price'], $_SESSION[$session]['orders'][$i]['qty'], $_SESSION[$session]['orders'][$i]['opt']);
        $orderfields = array( 'numorder' => $numorder,
  ...
  if (empty($tsvshop['SmtpFromEmail'])) {$tsvshop['SmtpFromEmail']=$tsvshop['youremail'];}
