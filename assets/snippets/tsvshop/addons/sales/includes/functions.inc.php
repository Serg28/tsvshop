    ...
    if ($user == "manager") {
        if (!empty($act) && $act == "delorder" && !empty($idorder) && is_numeric(intval($idorder))) {
            if ($modx->db->query("
       		...
       		")) {
                $output_sales_notice .= $shop_lang['sales_del_ok'];
            } else {
                $output_sales_error .= $shop_lang['sales_del_error'];
            };
            ...
            if ($output_sales_notice <> "") {
                $output = notice($shop_lang['sales_del_ok'], 'success');
            }
            if ($output_sales_error <> "") {
                $output = notice($shop_lang['sales_del_error'], 'error');
            }
        }
    }
    return $output;
...
function clearorder()
{
    ...
    if ($user == "manager") {
        if (!empty($act) && $act == "clearorders") {
			...
			")) {
                $output_sales_notice .= $shop_lang['sales_clear_ok'];
            } else {
                $output_sales_error .= $shop_lang['sales_clear_error'];
            }
            ...
            if ($output_sales_notice <> "") {
                $output = notice($shop_lang['sales_clear_ok'], 'success');
            }
            if ($output_sales_error <> "") {
                $output = notice($shop_lang['sales_clear_error'], 'error');
            }
        }
        return $output;
    }
        ...
        if (!empty($act) && $act == "delchecked") {
            foreach ($_GET as $val) {
                if ($val <> "check_del" && is_numeric($val)) {
					...
					")) {
                        $output_sales_notice .= $shop_lang['sales_check_del_ok'];
                    } else {
                        $output_sales_error .= $shop_lang['sales_check_del_error'];
                    };
            ...
            if ($output_sales_notice <> "") {
                $output = notice($shop_lang['sales_check_del_ok'], 'success');
            }
            if ($output_sales_error <> "") {
                $output = notice($shop_lang['sales_check_del_error'], 'error');
            }
        }
        return $output;
    }
...
function updateorder($idorder)
{
    ...
    if (!is_array($tsvshop['sysfields'])) {
        $tsvshop['sysfields'] = explode(',', $tsvshop['sysfields']);
    }
    if (!is_array($tsvshop['customfields'])) {
        $tsvshop['customfields'] = explode(',', $tsvshop['customfields']);
    }
    $tsvshop['sysfields'] = array_merge($tsvshop['sysfields'], $tsvshop['customfields']);
    foreach ($folders as $folder) {
        if ($folder != "." && $folder != "..") {
            if (!is_array($tsvshop['cf_' . $folder])) {
                $tsvshop['cf_' . $folder] = explode(',', $tsvshop['cf_' . $folder]);
            }
...
switch ($_GET['act']) {
    case 'delorder':
        $snotice .= delorder($_GET['idorder']);
        break;
    case 'clearorders':
        $snotice .= clearorder();
        break;
    case 'delchecked':
        $snotice .= checkdelorder();
        break;
    case 'view':
        exit(vieworder($_GET['idorder']));
        break;
    case 'updateorder':
        exit(updateorder($_GET['idorder']));
        break;
    case 'updstorder':
        exit(updstorder($_GET['idorder']));
        break;
}
