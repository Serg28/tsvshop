...
if(!$isPostBack){
    ...
    $comments = $modx->db->escape($modx->stripTags($_POST['comments']));
    ...
    if($username=="") {
        ...
        $output = "<div class=\"error\">Вы не ввели Логин пользователя. Пожалуйста, введите его и попробуйте снова.</div>".$tpl;
        return;
    }
    else {
        ...
        if(!$rs = $modx->db->query($sql)){
            ...
            $output = "<div class=\"error\">Извините, но произошла ошибка во время поиска пользователей с Логином $username. Попробуйте регистрацию еще раз. Если ошибка повториться, попробуйте зарегистрироваться чуть позже.".$tpl;
            return;
        }
        $limit = $modx->db->getRecordCount($rs);
        if($limit>0) {
            //$output = webLoginAlert("Username is already in use!").$tpl;
            $output = "<div class=\"error\">Такой Логин уже используется!</div>".$tpl;
            return;
        }
    }
    ...
    if($email=='' || !preg_match("/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,6}$/i", $email)){
        ...
        $output = "<div class=\"error\">E-mail адрес имеет неправильный формат!</div>".$tpl;
        return;
    }
    ...
    if(!$rs = $modx->db->query($sql)){
        ...
        $output = "<div class=\"error\">Извините, но произошла ошибка во время поиска пользователя с email-адресом $email. Попробуйте регистрацию еще раз. Если ошибка повториться, попробуйте зарегистрироваться чуть позже.</div>".$tpl;
        return;
    }
    $limit = $modx->db->getRecordCount($rs);
    if($limit>0) {
        ...
        if($row['internalKey']!=$id) {
            ...
            $output = "<div class=\"error\">Email адрес, введенный вами, уже используется!</div>".$tpl;
            return;
        }
    }
 	...
 	if (isset($_POST['password'])) {
		// verify password
 	    if ($_POST['password'] != $_POST['confirmpassword']) {
 	  		...
 	  		$output = "<div class=\"error\">Пароли, введенные вами в обоих полях, не идентичны. Пожалуста, проверьте их и попробуйте еще раз</div>". $tpl;
 	  	    return;
 	  	}
	    ...
	    if (strlen($password) < 6 ) {
	        ...
	        $output = "<div class=\"error\">Выбранный вами пароль очень короткий. Он не должен быть короче 6 символов!</div>".$tpl;
	        return;
	    }
	    elseif($password=="") {
	        //$output = webLoginAlert("You didn't specify a password for this user!").$tpl;
	        $output = "<div class=\"error\">Вы не указали пароль для этого пользователя!</div>".$tpl;
	        return;
	    }
 	} else {
    ...
    if($useCaptcha && $_SESSION['veriword']!=$formcode) {
        ...
        $output = "<div class=\"error\">Неправильный код проверки. Пожалуйста, введите правильный код, показанный на картинке.</div>".$tpl;
        return;
    }
    ...
    if(!$rs){
        ...
        $output = "<div class=\"error\">Извините, но во время сохранения пользователя возникла ошибка. Попробуйте регистрацию еще раз. Если ошибка повториться, попробуйте зарегистрироваться чуть позже.</div>".$tpl;
        return;
    }
    // now get the id
    ...
    $sql = "INSERT INTO ".$modx->getFullTableName("web_user_attributes")." (internalKey, fullname, email, zip, state, phone, country, type, blocked, comments)
            VALUES($key, '$fullname', '$email', '$zip', '$state', '$phone', '$country', '$type', '1', '$comments');";
    $rs = $modx->db->query($sql);
    if(!$rs){
        //$output = webLoginAlert("An error occured while attempting to save the user's attributes.").$tpl;
        $output = "<div class=\"error\">Извините, но во время сохранения параметров пользователя произошла ошибка. Попробуйте регистрацию еще раз. Если ошибка повториться, попробуйте зарегистрироваться чуть позже.</div>".$tpl;
        return;
    }
    ...
    if(count($groups)>0) {
        ...
        if(!$ds) return /*$modx->webAlert('An error occured while attempting to update user\'s web groups');*/ '<div class="error">Возника ошибка при добавлении пользователя в группу</div>';
        else {
    ...
    $newpassmsg = "<div class=\"notice\">Копия вашего пароля была выслана вам на электронный адрес.</div>";
    $tpl = $tpls[1];
